...
export default function Index({ tags, filters }) {
    ...
    const handleSearch = (e) => {
        e.preventDefault();
        get(route('admin.blog.tags.index'), { preserveState: true });
    };

    return (
        <AuthenticatedLayout header="Tags do Blog">
            <Head title="Tags" />
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                <div className="p-6 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    ...
                    <Link href={route('admin.blog.tags.create')} className="px-4 py-2 bg-[#557bbb] text-white text-sm font-semibold rounded-lg hover:bg-[#009b80] transition-colors shadow-sm whitespace-nowrap flex items-center">
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-sm text-left text-gray-600">
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {tags.data.length > 0 ? (
                                tags.data.map(tag => (
                                    <tr key={tag.id} className="bg-white hover:bg-gray-50/80 transition-colors group">
                                        ...
                                        <td className="px-6 py-4 text-right">
                                            <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <Link href={route('admin.blog.tags.edit', tag.id)} className="p-1.5 text-blue-600 hover:text-blue-800 bg-blue-50 hover:bg-blue-100 rounded-md transition-colors" title="Editar">
                                                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                                </Link>
                                                <Link href={route('admin.blog.tags.destroy', tag.id)} method="delete" as="button" className="p-1.5 text-red-500 hover:text-red-700 bg-red-50 hover:bg-red-100 rounded-md transition-colors" title="Excluir">
