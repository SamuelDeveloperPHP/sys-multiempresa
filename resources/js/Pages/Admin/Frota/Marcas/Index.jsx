...
export default function MarcasIndex({ marcas, filtros = {} }) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => router.get(route('admin.frota.marcas.index'), { q: q || undefined }, { preserveState: true, preserveScroll: true }), 350);
    return () => clearTimeout(t);
  }, [q]); // eslint-disable-line react-hooks/exhaustive-deps
  const limpar = () => { setQ(''); router.get(route('admin.frota.marcas.index'), {}, { preserveScroll: true }); };

  const salvarNovo = (e) => { e.preventDefault(); novo.post(route('admin.frota.marcas.store'), { preserveScroll: true, onSuccess: () => novo.reset() }); };
  const excluir = (m) => { if (!confirm(`Remover a marca "${m.marca}"?`)) return; router.delete(route('admin.frota.marcas.destroy', m.id), { preserveScroll: true }); };
    ...
    <AuthenticatedLayout>
      ...
      <div className="p-4 md:p-6 w-full">
        ...
        <form onSubmit={salvarNovo} className="bg-white rounded-lg border p-4 mb-4">
          <h2 className="font-semibold mb-2 text-sm text-gray-700">Adicionar marca</h2>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-2">
            <input value={novo.data.marca} onChange={(e) => novo.setData('marca', e.target.value)} placeholder="Nome da marca *" className="md:col-span-3 border border-gray-300 rounded px-3 py-2 text-sm" />
        ...
        <div className="bg-white rounded-lg border overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 text-left"><tr>
              <th className="px-3 py-2">Marca</th><th className="px-3 py-2 text-right">Modelos</th><th className="px-3 py-2 text-right">Ações</th>
            </tr></thead>
            <tbody className="divide-y">
              {marcas.data.length === 0 ? (
                <tr><td colSpan={3} className="text-center text-gray-500 py-8">Nenhuma marca cadastrada.</td></tr>
              ) : marcas.data.map((m) => (
                <Row key={m.id} marca={m} editing={editingId === m.id} onEdit={() => setEditingId(m.id)} onCancel={() => setEditingId(null)} onDelete={() => excluir(m)} />
              ))}
            </tbody>
          </table>
        </div>

        {marcas.links?.length > 3 && (
...
function Row({ marca, editing, onEdit, onCancel, onDelete }) {
  ...
  const salvar = (e) => { e.preventDefault(); edit.put(route('admin.frota.marcas.update', marca.id), { preserveScroll: true, onSuccess: onCancel }); };

  if (!editing) {
    return (
      <tr className="hover:bg-gray-50">
        <td className="px-3 py-2 font-medium text-gray-800">{marca.marca}</td>
        <td className="px-3 py-2 text-right">{marca.modelos_count}</td>
  ...
  return (
    <tr className="bg-amber-50">
      <td className="px-3 py-2"><input value={edit.data.marca} onChange={(e) => edit.setData('marca', e.target.value)} className="w-full border rounded px-2 py-1 text-sm" /></td>
      <td className="px-3 py-2 text-right">{marca.modelos_count}</td>
